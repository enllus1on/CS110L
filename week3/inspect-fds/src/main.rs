...
fn main() {
    ...
    // unimplemented!();
    let target = ps_utils::get_target(target)
        .expect("ps/pgrep error!");

    if target.is_some() {
        let target = target.unwrap();
        target.print();
        let childs = get_child_processes(target.pid).expect("failed to get child processes");
        for child in &childs {
            child.print();
        }
    }
    else {
        println!("failed to get \"{}\" pid!", &args[1]);
        std::process::exit(1);
    }
